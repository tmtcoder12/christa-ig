...
from christa_ig.config import get_settings
from christa_ig.http_client import perform_request
from christa_ig.observability import log_event, request_id
from christa_ig.security import secure_equals, verify_meta_signature
from christa_ig.webhook_events import expand_meta_events
...
def get_allowed_frontend_origins():
    return set(get_settings().frontend_origins)
...
def send_twilio_sms(to_phone_e164, body):
    ...
    try:
        response = perform_request(api_request)
        ...
        return {"success": False, "error": str(getattr(exc, "reason", exc))}
...
def store_comment_classification(comment_id, business_id, classifier_result, trigger_mode, threshold):
    ...
    try:
        ...
        log_event(
            logger,
            "comment_classification_store_failed",
...
def parse_db_timestamp(value):
    ...
    try:
        ...
        log_event(logger, "invalid_database_timestamp", level=logging.WARNING)
        return None
...
def dependency_error(dependency, error, status=503):
    log_event(
        logger,
    ...
    return api_error(
        f"{dependency.capitalize()} is temporarily unavailable",
        status,
...
def verify_supabase_user_token(token):
    ...
    try:
        response = perform_request(api_request, retry_safe=True)
        return json.loads(response.body.decode("utf-8"))
    except urllib.error.HTTPError as exc:
        exc.read()
        log_event(logger, "supabase_auth_verification_failed", level=logging.WARNING, status_code=exc.code)
        return None
...
def validate_promotion_payload(payload):
    ...
    if len(raw_keywords) > 100:
        raise ValueError("trigger_keywords must contain 100 items or fewer")
    ...
    for keyword in raw_keywords:
        ...
        if len(stripped) > 100:
            raise ValueError("Each trigger keyword must be 100 characters or fewer")
    ...
    if len(comment_reply_text) > MAX_PUBLIC_COMMENT_REPLY_CHARS:
        raise ValueError(f"comment_reply_text must be {MAX_PUBLIC_COMMENT_REPLY_CHARS} characters or fewer")

    dm_prompt = str(payload.get("dm_prompt") or "").strip() or None
    if dm_prompt and len(dm_prompt) > 2000:
        raise ValueError("dm_prompt must be 2000 characters or fewer")
    code_prefix = str(payload.get("code_prefix") or "").strip() or None
    if code_prefix and len(code_prefix) > 20:
        raise ValueError("code_prefix must be 20 characters or fewer")
...
def fetch_instagram_media_item(media_id):
    ...
    try:
        response = perform_request(api_request, retry_safe=True)
        media_item = json.loads(response.body.decode("utf-8"))
    except urllib.error.HTTPError as exc:
        error_body = exc.read().decode("utf-8", errors="replace")
...
def newest_unseen_media(media_items, baseline_media_ids):
    ...
    candidates = [
        item for item in media_items if isinstance(item, dict) and item.get("id") and item["id"] not in baseline
    ]
    candidates.sort(
...
def process_promotion_setup_tick(setup):
    """Process one durable poll attempt; the worker schedules the next tick."""
    setup_id = setup["id"]
    now = datetime.now(timezone.utc)
    expires_at = parse_db_timestamp(setup.get("poll_expires_at"))
    if expires_at and now > expires_at:
        return update_promotion_setup(
            setup_id,
            {
                "status": "expired",
    ...
    if not account or account.get("status") != "connected":
        return update_promotion_setup(
            ...
            {
                "status": "error",
                "error_message": "Instagram account is missing or not connected",
                "locked_at": None,
                "locked_until": None,
                "locked_by": None,
            },
    ...
    media_item = newest_unseen_media(
        fetch_instagram_media(account["instagram_user_id"]),
        setup.get("baseline_media_ids") or [],
    )
    if media_item:
    ...
    return update_promotion_setup(
        setup_id,
        {
            "status": "polling",
            "last_polled_at": now.isoformat(),
            "next_poll_at": (now + timedelta(seconds=PROMOTION_POLL_INTERVAL_SECONDS)).isoformat(),
            "locked_at": None,
            "locked_until": None,
            "locked_by": None,
        },
...
def run_promotion_setup_poll(setup_id):
    """Backward-compatible single-tick wrapper used by older callers."""
    setup = get_promotion_setup(setup_id)
    if not setup or setup.get("status") not in {"pending", "polling"}:
        return None
    return process_promotion_setup_tick(setup)


def start_promotion_setup_poll(setup_id):
    """Mark a setup ready for the durable worker; no thread is created."""
    return update_promotion_setup(
        setup_id,
...
def handle_promo_lead_capture(
    instagram_account, contact, session, history, message_text, inbound_message, event_type, payload
):
    ...
    upsert_meta_webhook_event(
        event_id=(inbound_message.get("instagram_message_id") if inbound_message else None)
        or (inbound_message.get("id") if inbound_message else None),
        ...
        error_message=error_message
        or (None if send_message_response["success"] else send_message_response.get("error")),
    )
...
def process_comment_with_database(comment_info, payload, event_type):
    ...
    if not post:
        ...
        try:
            ...
            log_event(
                logger,
                "instagram_media_discovery_failed",
                level=logging.WARNING,
                media_id=media_id,
                error_type=type(exc).__name__,
            )
    ...
    if not trigger_result.get("matched"):
        ...
        return {
            ...
            "db_result": {
                ...
                "classification_id": (trigger_result.get("classification_row") or {}).get("id"),
            },
        }
...
def health_ready():
    ...
    checks = {
        "configuration": True,
        "meta": bool(settings.meta_verify_token and settings.meta_app_secret),
...
def verify_webhook():
    ...
    if mode == "subscribe" and secure_equals(expected_token, verify_token):
        log_event(logger, "meta_webhook_verification", outcome="succeeded")
    ...
    log_event(
        ...
        expected_token_set=bool(expected_token),
    )
...
def twilio_sms_webhook():
    ...
    if not is_valid:
        log_event(logger, "twilio_webhook_rejected", level=logging.WARNING, reason=validation_error)
        return Response("Forbidden", status=403, mimetype="text/plain")

    if not is_supabase_configured():
        log_event(logger, "twilio_webhook_ignored", level=logging.WARNING, reason="database_unavailable")
        return empty_twiml_response()

    form_payload = request.form.to_dict(flat=True)
    try:
        result = process_twilio_sms_reply(form_payload)
        log_event(logger, "twilio_webhook_processed", outcome=result.get("processing_result"))
        return empty_twiml_response()
    except SupabaseError as exc:
        log_event(logger, "twilio_webhook_failed", level=logging.ERROR, error_type=type(exc).__name__)
...
def validate_knowledge_chunk_payload(payload):
    ...
    if len(text) > 20_000:
        raise ValueError("text must be 20000 characters or fewer")
    ...
    optional_values = {name: clean_optional_string(payload.get(name)) for name in optional_limits}
    for name, limit in optional_limits.items():
        if optional_values[name] and len(optional_values[name]) > limit:
...
def get_knowledge_chunks_api():
    ...
    try:
        ...
        return dependency_error("supabase", exc)
...
def create_knowledge_chunk_api():
    ...
    try:
        ...
        log_event(logger, "knowledge_embedding_failed", level=logging.ERROR, error_type=type(exc).__name__)
        return api_error("Failed to generate embedding", 502, code="embedding_failed")
...
def is_followup_cron_authorized():
    ...
    return secure_equals(expected_secret, provided_secret)
...
def redeem_promo_code_api():
    ...
    try:
        ...
        phone_e164 = (lead or {}).get("phone_e164") or (contact or {}).get("phone_e164")
        display_name = (
            (lead or {}).get("customer_name") or (contact or {}).get("display_name") or (contact or {}).get("username")
        )
        ...
        return dependency_error("redemption", exc)
...
def create_promotion():
    ...
    try:
        ...
        return dependency_error("instagram", exc, status=502)
...
def process_webhook_payload(payload):
    ...
    if is_supabase_configured() and event_type in {"dm-related", "comment-related"}:
        try:
            ...
            log_event(logger, "promo_code_expiration_failed", level=logging.ERROR)
        ...
        if dm_info["should_reply"]:
            try:
                if is_supabase_configured():
                    ...
                    log_event(logger, "dm_in_memory_fallback", level=logging.WARNING)
                ...
                log_event(logger, "instagram_dm_processing_failed", level=logging.ERROR, error_type=type(exc).__name__)
        ...
        if comment_info["should_process"]:
            try:
                if is_supabase_configured():
                    ...
                    log_event(
                        logger, "comment_automation_disabled", level=logging.WARNING, reason="database_unavailable"
                    )
                    processing_result = "comment_automation_requires_database"
            except SupabaseError as exc:
                log_event(
                    logger, "instagram_comment_processing_failed", level=logging.ERROR, error_type=type(exc).__name__
                )
                processing_result = "db_error"
                db_result = {"error": str(exc)}
    ...
    return {
        "event_type": event_type,
        "processing_result": processing_result,
        "openai_success": openai_result.get("success") if openai_result else None,
        "send_success": send_message_response.get("success") if send_message_response else None,
        "public_reply_success": public_reply_response.get("success") if public_reply_response else None,
        "private_reply_success": private_reply_response.get("success") if private_reply_response else None,
        "database_ids": {
            key: value
            for key, value in (db_result or {}).items()
            if key.endswith("_id") or key in {"business_id", "instagram_account_id"}
        },
        "expired_promo_code_count": len(expired_promo_codes),
    }
...
def webhook():
    ...
    if not verify_meta_signature(raw_body, signature, settings.meta_app_secret):
        log_event(logger, "meta_webhook_delivery", level=logging.WARNING, outcome="invalid_signature")
        return "Unauthorized", 401, {"Content-Type": "text/plain; charset=utf-8"}

    payload = request.get_json(silent=True)
    jobs = expand_meta_events(payload)
    if not jobs:
        log_event(logger, "meta_webhook_delivery", outcome="ignored", reason="no_supported_events")
        return "OK", 200, {"Content-Type": "text/plain; charset=utf-8"}
    if not is_supabase_configured():
        log_event(
            logger,
            "meta_webhook_delivery",
            level=logging.ERROR,
    ...
    try:
        inserted = enqueue_webhook_jobs(jobs, request_id=request_id())
    except SupabaseError as exc:
        log_event(
            logger,
            "meta_webhook_delivery",
    ...
    log_event(
        ...
        event_ids=[job["external_event_id"] for job in jobs],
    )
    return "OK", 200, {"Content-Type": "text/plain; charset=utf-8"}


def _handle_unexpected_error(error):
    log_event(logger, "unhandled_request_error", level=logging.ERROR, error_type=type(error).__name__)
    return api_error("Internal server error", 500, code="internal_error")


def _handle_request_too_large(_error):
    return api_error("Request body is too large", 413, code="request_too_large")


def _handle_http_error(error):
    if request.path.startswith("/api/"):
        return api_error(error.description, error.code, code=f"http_{error.code}")
    return error
